...
class DetailMedCetakTabloidRelationManager extends RelationManager
{
    protected static string $relationship = 'permohonanDetMedKomCetak';
    const KEGIATAN_ID = 'cd6d6a81-e472-480b-8bf4-7144c0e75ed7';
    ...
    {
        return static::getTargetKegiatan()?->nama ?? 'Tabloid';
    }
                ...
                Grid::make(2)
                    ->schema([
                        TextInput::make('jumlah')
                            ...
                            ->minValue(0)
                            ->live(onBlur: true)
                            ->afterStateUpdated(fn(Get $get, Set $set) => self::updateVolumeTabloid($get, $set))
                            // ->afterStateUpdated(
                                // dd('sasasasa'),
                                // fn(Get $get, Set $set) => $set('volume_hitung', $get('jumlah')),
                                // fn(Get $get, Set $set) => dd('8888'),
                            // )
                            ,
                        TextInput::make('volume_hitungs'),
    ...
    public static function updateVolumeTabloid(Get $get, Set $set): void
    {
        $jumlah = (int) ($get('jumlah') ?? 0);
        $set('volume_hitungs', $jumlah);
    }
